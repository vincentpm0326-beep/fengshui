...
function trackEvent(name, props){
  try{
    fetch('/api/event', {
      ...
      body:JSON.stringify({name:name, props:props||{}, path:location.pathname+location.hash})
    }).catch(function(){});
  }catch(e){}
}
...
function getMembership(){
  try{
    var m=JSON.parse(localStorage.getItem('cma_membership')||'null');
...
function membershipDaysLeft(){
  var m=getMembership();
  if(!m)return 0;
  return Math.max(0, Math.ceil((new Date(m.expiresAt).getTime()-Date.now())/86400000));
}
...
function activateMembership(planKey){
  var plan=MEMBERSHIP_PLANS[planKey]||MEMBERSHIP_PLANS.monthly;
  var now=new Date();
  var current=getMembership();
  var base=current && new Date(current.expiresAt).getTime()>Date.now() ? new Date(current.expiresAt) : now;
  ...
  saveMembership({
    ...
    lightCredits:(current&&current.lightCredits||0)+plan.lightCredits
  });
  trackEvent('membership_open', {plan:planKey, days:plan.days, light_credits:plan.lightCredits});
}

function consumeMembershipLightCredit(){
  var m=getMembership();
  if(!membershipActive() || !m.lightCredits) return false;
  m.lightCredits = Math.max(0, (m.lightCredits||0)-1);
  saveMembership(m);
  return true;
}

function isLightServiceContext(){
  var active=document.querySelector('.page.on');
  if(!active)return false;
  return ['page-ask','page-dream','page-almanac'].indexOf(active.id)>=0;
}
...
function refundProAccess() {
  ...
  else if (LAST_DEDUCTION === 'member-light') { var m=getMembership(); if(m){m.lightCredits=(m.lightCredits||0)+LAST_DEDUCTION_COUNT; saveMembership(m);} }
...
function showPaywall() {
  ...
  trackEvent('paywall_view', {member:membershipActive(), credits:PRO_CREDITS});
}
...
function selectMemberPlan(card, planKey) {
  SELECTED_MEMBERSHIP_PLAN = planKey || 'monthly';
  document.querySelectorAll('.member-plan-card').forEach(function(c){ c.style.outline='none'; c.style.boxShadow='none'; });
  if(card){
    card.style.outline = '1.5px solid var(--gold)';
    card.style.boxShadow = '0 0 0 3px rgba(201,168,76,0.18)';
  }
  var plan=MEMBERSHIP_PLANS[SELECTED_MEMBERSHIP_PLAN]||MEMBERSHIP_PLANS.monthly;
  var hint = document.getElementById('member-plan-hint');
  if(hint){hint.style.display='block';hint.textContent='已选：'+plan.name+' · '+plan.price+' · '+plan.desc;}
  trackEvent('member_plan_select', {plan:SELECTED_MEMBERSHIP_PLAN});
}

function simulateMemberOpen(planKey){
  activateMembership(planKey || SELECTED_MEMBERSHIP_PLAN || 'monthly');
  var msg=document.getElementById('member-open-msg');
  var m=getMembership();
  if(msg){
    msg.textContent='已开通 '+m.name+'，有效期至 '+new Date(m.expiresAt).toLocaleDateString('zh-CN');
  ...
  fetch('/api/activate', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({code})})
    ...
    .then(function(d){
      if (d.ok) {
        ...
        trackEvent('activate_fail', {message:d.message||'fail'});
      }
    })
...
function saveHistory(type, title, score, data) {
  try {
    ...
    renderReportCenter();
    updateDailyRetention();
    trackEvent('report_saved', {type:type, title:title, score:score||''});
  } catch(e){}
}
...
function addReminder(title, type, dueDate, note, sourceId){
  ...
  saveReminders(list);
  trackEvent('reminder_created', {type:type||'事项提醒', dueDate:dueDate||'', source:sourceId?'report':'manual'});
  alert('已加入提醒中心');
}
...
function renderMemberCenter(){
  var title=document.getElementById('member-status-title');
  if(!title)return;
  var m=getMembership();
  var active=membershipActive();
  var days=membershipDaysLeft();
  var reminders=loadReminders().filter(function(r){return r.status!=='done';}).length;
  title.textContent=active ? (m.name+' · 已开通') : '暂未开通会员';
  document.getElementById('member-status-text').textContent=active ?
    '会员权益生效中：每日提醒、轻问事/解梦权益和报告提醒会持续为你保留。专题深度服务仍可单独购买。' :
    '开通后可获得每日黄历不限看、每日个人提醒、轻问事/解梦权益和报告提醒能力。';
  document.getElementById('member-days-left').textContent=days;
  document.getElementById('member-light-left').textContent=active ? (m.lightCredits||0) : 0;
  document.getElementById('member-reminder-count').textContent=reminders;
}
...
function goTo(p){
  ...
  if(p==='member')renderMemberCenter();
...
function reportUsage(){
  trackEvent('analysis_success', {module:(document.querySelector('.page.on')||{}).id||'', member:membershipActive(), credits:PRO_CREDITS});
...
document.getElementById('ask-btn').addEventListener('click',function(){
  ...
  trackEvent('analysis_start', {module:'quick_ask', category:cls.category, service_type:'light'});
...
document.getElementById('generate-btn').addEventListener('click',function(){
  ...
  trackEvent('analysis_start', {module:'fengshui', service_type:hasImages?'space_image':'space_text'});
...
document.getElementById('dreambtn').addEventListener('click',function(){
  ...
  if(!checkProAccess())return;
  trackEvent('analysis_start', {module:'dream', service_type:'light', has_text:!!txt});
  noerr('dream-err');
...
document.getElementById('alm-btn').addEventListener('click',function(){
  if(!checkProAccess())return;
  trackEvent('analysis_start', {module:'almanac', service_type:'light'});
  var now=new Date();
...
document.getElementById('profilebtn').addEventListener('click',function(){
  ...
  if(!checkProAccess())return;
  trackEvent('analysis_start', {module:'bazi', service_type:'special_topic'});
...
document.getElementById('w-btn').addEventListener('click',function(){
  ...
  if(!checkProAccess())return;
  trackEvent('analysis_start', {module:'wealth', service_type:'special_topic'});
